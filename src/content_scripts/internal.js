(function () {
    ...
    function mutationListener() {
        ...
        if (!isMenuElementInserted) {
            ...
            chrome.runtime.sendMessage(EXTENSION_ID, {action: "listenContestAdvShow"});
        }
        ...
        if (sessionStorage.listenContest == 1) {
            ...
            chrome.runtime.sendMessage(EXTENSION_ID, {action: "listenContestAdvClick"});

            var advHTML = (!/^en/i.test(navigator.language))
                ? 'Информация о конкурсе показана с помощью приложения VK Offline для Google Chrome. Если вы больше не хотите ее видеть, нажмите <a href="javascript:;" onclick="preventAd(); return false">здесь</a>.'
                : 'Contest advertisement is shown by VK Offline app for Google Chrome. If you don\'t want to see it again, click <a href="javascript:;" onclick="preventAd(); return false">here</a>.';
