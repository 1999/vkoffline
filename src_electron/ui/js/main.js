...
chrome.runtime.onMessage.addListener(request => {
    switch (request.action) {
        case 'useInitialSettings':
            // initial settings should applied only one time
            if (window.Settings || window.Account) {
                return;
            }

            window.Settings = request.flatSettings;
            window.Account = request.accountData;
            break;
...
// settings and account data are stored inside sync page
chrome.runtime.sendMessage({action: 'getInitialSettings'});
